...
void main() {
  test('when call ChangePasswordRepository it should call it from online data source for change password', () async {
    const oldPassword = "Old@1234";
    const newPassword = "New@1234";
    const rePassword = "New@1234";
    const token = "Bearer token";

    var mockChangePasswordOnLineDataSource=MockChangePasswordOnlineDataSource();
    var changePasswordRepository=ChangePasswordRepositoryImpl(mockChangePasswordOnLineDataSource);

    var changePassword=ChangePasswordEntity();
    var mockedResult=Success<ChangePasswordEntity>(changePassword);
    //Stub
    when(mockChangePasswordOnLineDataSource.changePassword(oldPassword,newPassword,rePassword,token)).thenAnswer((_)async=>mockedResult);
    //Act
    var result= await changePasswordRepository.changePassword(oldPassword,newPassword,rePassword,token);
    //assert
    verify(mockChangePasswordOnLineDataSource.changePassword(oldPassword,newPassword,rePassword,token)).called(1);
