...
void main() {
  test("when call invoke it should call ChangePasswordRepository",
          () async {
        //arrange
        ...
        var changePasswordUseCase=ChangePasswordUseCase(mockChangePasswordRepository);
        var changePassword=ChangePasswordEntity();
        var mockedResult=Success<ChangePasswordEntity>(changePassword);
        //Stub
        when(mockChangePasswordRepository.changePassword(oldPassword, newPassword, rePassword, token)).thenAnswer((_)async=>mockedResult);
        //Act
        
        var result= await changePasswordUseCase.invoke(oldPassword,newPassword,rePassword,token);
        //assert
        verify(mockChangePasswordRepository.changePassword(oldPassword, newPassword, rePassword, token)).called(1);
